...
### 📄 模組簡介
本飼料包（Training Feed）專為**極端資料安全與隱私合規**設計。它將代理的 I/O 流控升級為「先過濾後傳輸」的真實狀態機 (True State Machine)，強制綑綁 2026 最新法規，確保在讀取、總結或外部傳輸任何資料前，絕對攔截並屏蔽所有個人可識別資訊 (PII)。

### ⚙️ 技能協同 (Skill Synergy)
- **建議搭配**：適用於任何讀寫操作，特別是 `read_url`, `read_terminal`, `search_web`
- **協同效應**：代理在獲取或準備輸出資料時必須啟動自檢狀態機。若偵測到未加密的 PII，將觸發 Validation 立即覆蓋，或觸發 Failure 中止操作。
