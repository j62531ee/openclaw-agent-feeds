...
### 📄 模組簡介
本飼料包（Mutation Feed）專為**安全地生成與修改專案架構**設計。它將您的代理升級為具備「防呆狀態機」的高階工程師。在寫入任何程式碼前，它必須經過極度嚴謹的 Assess, Backup, Minimal Diff 與 Validation 流程，徹底杜絕把原本正常的專案改壞的悲劇。

### ⚙️ 技能協同 (Skill Synergy)
- **建議搭配**：`run_command`, `replace_file_content`, `list_dir`
- **協同效應**：將檔案修改工具與嚴格的查核狀態機結合。所有 `write`/`replace` 動作都被狀態機的 `Verify` 與 `Rollback` 分支嚴密監控。
